    ...
    public partial class FlatSearch : FlatBasePage
    {
        ...
        private void DoSearch()
        {
         ...
         bool bAutoSkip = (GetPicklistMoniker() == null);
            ...
            try
            {
                IAddressLookup searchService = QAS.GetSearchService(); 
                searchService.CurrentEngine.EngineType = Engine.EngineTypes.Singleline;
                searchService.CurrentEngine.Flatten = true;

                if (bAutoSkip)
                {
                    m_Picklist = searchService.Search(GetDataID(), GetInputLine(), GetPromptSet(), GetLayout()).Picklist;

                    while ( m_Picklist.IsAutoStepinSafe )
                    {
                        m_Picklist = searchService.StepIn(m_Picklist.PicklistItems[0].Moniker, GetLayout());
                    }
                ...
                else
                {
                    ...
                    m_Picklist = searchService.StepIn(GetPicklistMoniker(), GetLayout());
                }
            ...
            else if (bAutoSkip)
            {
                ...
                else if (m_Picklist.PicklistItems.Count == 1 && MustRefine(m_Picklist.PicklistItems[0]))
                {
                    // Auto-step past picklist to refinement page
                    GoRefinementPage(m_Picklist.PicklistItems[0].Moniker);
                }
            }
