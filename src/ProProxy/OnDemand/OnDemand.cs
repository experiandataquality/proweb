namespace Experian.Qas.Proweb
{
    ...
    using System.Net;
    using System.Web;
    using Experian.Qas.OnDemand.soap;

    /// <summary>
    /// A class to manage our interactions with OnDemand.
    /// </summary>
    public class OnDemand : IAddressLookup
    {
        private Engine currentEngine = new Engine();

        /// <summary>
        /// Initializes a new instance of the <see cref="OnDemand" /> class.
        /// Create a new instance of OnDemand and instantiate the Proxy classes to the OnDemand service.
        /// </summary>
        /// <param name="endpointURL">The OnDemand endpoint.</param>
        /// <param name="username">You deployment username.</param>
        /// <param name="password">Your deployment password.</param>
        /// <param name="proxy">The proxy - if required.</param>
        /// <remarks>
        ...
        /// </remarks>
        public OnDemand(string endpointURL, string username, string password, IWebProxy proxy)
        {
            SearchService = new ProxyOnDemand();
            ...
            SearchService.QAQueryHeaderValue = header;

            if (proxy != null)
            {
                SearchService.Proxy = proxy;
            }
        }
        ...
        /// </summary>
        private ProxyOnDemand SearchService
        {
            get;
            set;
        }

        #region IAddressLookup Implemention
        ...
        /// <param name="layoutName">Layout to be used in the final address.</param>
        /// <param name="promptSet">Name of the Prompt Set to be used for searching.</param>
        ...
        /// <summary>
        /// Do not use. Bulk searching (supplying multiple addresses at once) is only available with our on 
        /// premise QAS Pro Web or hosted QAS Clean applications. 
        /// </summary>
        /// <param name="dataId">The parameter is not used.</param>
        /// <param name="searches">The parameter is not used.</param>
        /// <param name="promptSet">The parameter is not used.</param>
        /// <param name="layout">The parameter is not used.</param>
        /// <returns>NotImplementedException.</returns>
        public BulkSearchResult BulkSearch(string dataId, IList<string> searches, PromptSet.Types promptSet, string layout)
        {
            throw new NotImplementedException("Bulk Search is not available in OnDemand - QAS Pro Web only");
        }
        ...
        public PromptSet GetPromptSet(string dataId, PromptSet.Types prompt)
        {
            ...
            try
            {
                ...
                foreach (Experian.Qas.OnDemand.soap.PromptLine lineType in promptSetType.Line)
                {
        ...
        private static EngineType ConvertToEngineType(Engine engine)
        {
            ...
            switch (engine.Prompt)
            {
                ...
                case Engine.PromptSets.Generic:
                    engineType.PromptSet = PromptSetType.Generic;
                    break;
                case Engine.PromptSets.OneLine:
                    engineType.PromptSet = PromptSetType.OneLine;
                    break;
                case Engine.PromptSets.Optimal:
                    engineType.PromptSet = PromptSetType.Optimal;
                    break;
                case Engine.PromptSets.Alternate:
                    engineType.PromptSet = PromptSetType.Alternate;
                    break;
                case Engine.PromptSets.Alternate2:
                    engineType.PromptSet = PromptSetType.Alternate2;
                    break;
                case Engine.PromptSets.Alternate3:
                    engineType.PromptSet = PromptSetType.Alternate3;
                    break;
                default:
                    engineType.PromptSet = PromptSetType.Default;
                    break;
        ...
        private static PicklistItem CreatePicklistItem(PicklistEntryType entryType)
        {
            ...
            item.IsExtendedData = entryType.ExtendedData;
            item.IsEnhancedData = entryType.EnhancedData;
        ...
        /// </summary>
        /// <param name="x">The Exception to map.</param>
        private void MapException(Exception x)
        {
            ...
            {
                System.Web.Services.Protocols.SoapHeaderException header = x as System.Web.Services.Protocols.SoapHeaderException;
                throw x;
            }
            else if (x is System.Web.Services.Protocols.SoapException)
            {
                // Parse out qas:QAFault string
                System.Web.Services.Protocols.SoapException soap = x as System.Web.Services.Protocols.SoapException;
                System.Xml.XmlNode xmlDetails = soap.Detail;
                ...
                foreach (System.Xml.XmlNode xmlDetail in xmlDetails.ChildNodes)
                {
                    ...
                    {
                        message += xmlDetail.Name + ": [" + asDetail[1].Trim() + " " + asDetail[0].Trim() + "] ";
                    }
                    else
                    {
                        message += xmlDetail.Name + ": [" + asDetail[0].Trim() + "] ";
                    }
                }

                Exception exception = new Exception(message, soap);
                throw exception;
            }
            else
            {
